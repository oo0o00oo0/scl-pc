...
// Distance scaling uniforms
uniform float uScaleDistance;    // Distance at which scaling starts (default: 2.0)
uniform float uScaleFalloff;     // How quickly scaling falls off (default: 0.5)
...
// getDistanceScale:
// Returns a scale factor based on distance from origin.
// Splats near center (within uScaleDistance) have scale = 1.0
// Splats further away scale down exponentially based on uScaleFalloff
float getDistanceScale(vec3 position) {
  float distance = length(position);

  // No scaling if within the scale distance threshold
  if(distance <= uScaleDistance) {
    return 1.0;
  }

  // Exponential falloff for distances beyond threshold
  float excessDistance = distance - uScaleDistance;
  return exp(-excessDistance * uScaleFalloff);
}

// scaleSplatByDistance:
// Simple distance scaling function that takes splat points and multiplier
// Points close to center keep full scale, farther points scale down exponentially
float scaleSplatByDistance(vec3 splatPoint, float multiplier) {
  // Calculate distance from origin
  float distance = length(splatPoint);
  ...
  float scale = exp(-distance * multiplier);

  return max(0.01, scale); // Minimum scale to avoid invisible splats
}

// scaleSplatByDistanceWithThreshold:
// Alternative version with threshold - no scaling within certain distance
float scaleSplatByDistanceWithThreshold(vec3 splatPoint, float threshold, float multiplier) {
  // Calculate distance from origin
  float distance = length(splatPoint);
  ...
  float scale = exp(-excessDistance * multiplier);
...
void main(void) {
  ...
  vec3 modelCenter = readCenter(source);
    ...
    // 3. Calculate distance-based scale factor
  float distanceScale = getDistanceScale(modelCenter);

    // 4. Compute the center in view and clip space.
  SplatCenter center;
  if(!initCenter(modelCenter, center)) {
    ...
    // 5. Calculate the corner offset of the Gaussian.
  SplatCorner corner;
  ...
  float normalizedDistanceFromOrigin = length(corner.offset) * 0.03;
  ...
  gaussianColor = vec4(prepareOutputFromGamma(max(colMix.xyz, 0.0)), colMix.w * 1.0 * .2);
